...
public:
  ...
  Matrix4 &set(float, float, float, float, float, float, float, float, float,
  ...
  Matrix4 &lookAt2(const Vector3 &eye, const Vector3 &target,
                   const Vector3 &up);
  Matrix4 &setPerspective(float fov, float ratio, float near, float far);
  Matrix4 &setOrthogonal(float left, float right, float top, float bottom,
