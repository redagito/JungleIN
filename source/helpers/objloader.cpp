...
Mesh *ObjLoader::loadObj(QString fileName) {
    ...
    if (!list.isEmpty()) {
      if (list.at(0) == "v") {
        ...
        for (int i = 1; i < 4; ++i) {
          ...
          if (vertTex.at(subList.at(0).toInt() - 1).contains(-1) ||
              vertTex.at(subList.at(0).toInt() - 1)
                  .contains(subList.at(1).toInt() - 1)) { // cas normal
            ...
            if (duplicate.at(subList.at(0).toInt() - 1).at(0) ==
                subList.at(1).toInt() - 1) {
              face_index.pos_index[i - 1] =
                  duplicate.at(subList.at(0).toInt() - 1).at(1) + 1;
              face_index.tex_index[i - 1] = subList.at(1).toInt();
              face_index.nor_index[i - 1] = subList.at(2).toInt();
              ;
            } else {
              verticeList.append(
                  verticeList.at((subList.at(0).toInt() - 1) * 3));
              verticeList.append(
                  verticeList.at((subList.at(0).toInt() - 1) * 3 + 1));
              verticeList.append(
                  verticeList.at((subList.at(0).toInt() - 1) * 3 + 2));

              normalList.append(normalList.at((subList.at(2).toInt() - 1) * 3));
              normalList.append(
                  normalList.at((subList.at(2).toInt() - 1) * 3 + 1));
              normalList.append(
                  normalList.at((subList.at(2).toInt() - 1) * 3 + 2));
  ...
  for (int i = 0; i < faceCount; i++) {
    ...
    normalTab[posIndex0 * 3] =
        normalList.at((indiceList.at(i).nor_index[0] - 1) * 3);
    normalTab[posIndex0 * 3 + 1] =
        normalList.at((indiceList.at(i).nor_index[0] - 1) * 3 + 1);
    normalTab[posIndex0 * 3 + 2] =
        normalList.at((indiceList.at(i).nor_index[0] - 1) * 3 + 2);

    normalTab[posIndex1 * 3] =
        normalList.at((indiceList.at(i).nor_index[1] - 1) * 3);
    normalTab[posIndex1 * 3 + 1] =
        normalList.at((indiceList.at(i).nor_index[1] - 1) * 3 + 1);
    normalTab[posIndex1 * 3 + 2] =
        normalList.at((indiceList.at(i).nor_index[1] - 1) * 3 + 2);

    normalTab[posIndex2 * 3] =
        normalList.at((indiceList.at(i).nor_index[2] - 1) * 3);
    normalTab[posIndex2 * 3 + 1] =
        normalList.at((indiceList.at(i).nor_index[2] - 1) * 3 + 1);
    normalTab[posIndex2 * 3 + 2] =
        normalList.at((indiceList.at(i).nor_index[2] - 1) * 3 + 2);
  }
  ...
  for (int i = 0; i < faceCount; i++) {
    ...
    textTab[vertexPos0 * 2] =
        textList.at((indiceList.at(i).tex_index[0] - 1) * 2);
    textTab[vertexPos0 * 2 + 1] =
        textList.at((indiceList.at(i).tex_index[0] - 1) * 2 + 1);
    textTab[vertexPos1 * 2] =
        textList.at((indiceList.at(i).tex_index[1] - 1) * 2);
    textTab[vertexPos1 * 2 + 1] =
        textList.at((indiceList.at(i).tex_index[1] - 1) * 2 + 1);
    textTab[vertexPos2 * 2] =
        textList.at((indiceList.at(i).tex_index[2] - 1) * 2);
    textTab[vertexPos2 * 2 + 1] =
        textList.at((indiceList.at(i).tex_index[2] - 1) * 2 + 1);
  }
