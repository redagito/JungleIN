...
void MainWindow::initPaneParam() {
  ...
  QObject::connect(qualitySlider, SIGNAL(valueChanged(int)), paneGL,
                   SLOT(exQuality(int)));
  QObject::connect(postBtnFXAA, SIGNAL(toggled(bool)), paneGL,
                   SLOT(exPostFXAA(bool)));
  QObject::connect(postBtnCascade, SIGNAL(toggled(bool)), paneGL,
                   SLOT(exShadowCascade(bool)));
  QObject::connect(postBtnAniso, SIGNAL(toggled(bool)), paneGL,
                   SLOT(exAnisotropy(bool)));
  QObject::connect(postBtnFlares, SIGNAL(toggled(bool)), paneGL,
                   SLOT(exPostLenseFlares(bool)));
  QObject::connect(postBtnDistord, SIGNAL(toggled(bool)), paneGL,
                   SLOT(exPostDistord(bool)));
  QObject::connect(postBtnGodrays, SIGNAL(toggled(bool)), paneGL,
                   SLOT(exPostGodrays(bool)));
  QObject::connect(postBtnBloom, SIGNAL(toggled(bool)), paneGL,
                   SLOT(exPostBloom(bool)));
  QObject::connect(cameraBtnReset, SIGNAL(clicked()), paneGL,
                   SLOT(exResetCamera()));
  QObject::connect(ratioSlider, SIGNAL(valueChanged(int)), paneGL,
                   SLOT(exRenderRatio(int)));
  QObject::connect(sunSliderSpeed, SIGNAL(valueChanged(int)), paneGL,
                   SLOT(exSunSpeed(int)));
  QObject::connect(actionBtnFS, SIGNAL(clicked()), this,
                   SLOT(showFullScreen()));

  QObject::connect(btnPlay, SIGNAL(toggled(bool)), paneGL, SLOT(playBtn(bool)));
  QObject::connect(btnRegenerate, SIGNAL(clicked()), paneGL,
                   SLOT(reGenerate()));
}

QDockWidget *MainWindow::getDock() { return dock; }

QLCDNumber *MainWindow::getFPSDisplay() { return FPSDisplay; }
